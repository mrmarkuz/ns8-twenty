...
1. Click on [Use this template](https://github.com/NethServer/ns8-twenty/generate).
...
1. Rename some references inside the repo:
   ...
   git mv imageroot/systemd/user/twenty.service imageroot/systemd/user/${modulename}.service &&
   git mv imageroot/systemd/user/twenty-app.service imageroot/systemd/user/${modulename}-app.service && 
   git mv tests/twenty.robot tests/${modulename}.robot &&
   sed -i "s/twenty/${modulename}/g" $(find .github/ * -type f) &&
...
Instantiate the module with:
    ...
    add-module ghcr.io/nethserver/twenty:latest 1

The output of the command will return the instance name.
Output example:

    {"module_id": "twenty1", "image_name": "twenty", "image_url": "ghcr.io/nethserver/twenty:latest"}

## Configure

Let's assume that the mattermost instance is named `twenty1`.
...
```
api-cli run configure-module --agent module/twenty1 --data - <<EOF
{
  "host": "twenty.domain.com",
  "http2https": true,
  "lets_encrypt": false
}
EOF
...
- start and configure the twenty instance
...
```
api-cli run get-configuration --agent module/twenty1
```
